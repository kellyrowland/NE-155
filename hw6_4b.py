...
def GS(A,b,x_old,eps):
	...
	while delta > eps:
		n_iter += 1
		for i in range(0,n):
			x_new[i] = (b[i] - first_sum(i,A,x_new) - second_sum(i,A,x_old))/A[i,i]
...
n_iter = GS(A,S_matrix,phi_old,eps)[0]
...
e3b = np.array([37,131,2968,11733])
e5b = np.array([53,188,4297,16990])
e3c = np.array([20,57,1701,6827])
e5c = np.array([36,114,3029,12084])
...
plt.title("Number of Iterations vs. Mesh Size for Gauss Seidel Method")
plt.savefig('hw6_4b.pdf')
