...
B = np.array(np.linalg.inv(A)*F)
k, phi = np.linalg.eig(B)
print("eigenvalue: "+str(k[np.argmax(k)]))

k_old = 1.0
k_new = 1.0
phi_old = np.repeat(1.0, n)/np.linalg.norm(np.repeat(1.0, n))
phi_new = np.repeat(1.0, n)/np.linalg.norm(np.repeat(1.0, n))
...
phi_np = abs(phi[np.argmax(k)]/np.linalg.norm(phi[np.argmax(k)]))
x_range = np.linspace(x_min,x_max,num=n)
plt.plot(x_range, phi_np, color = 'b')
plt.plot(x_range, phi_plot, color = 'g')
plt.xlabel('x [cm]'); plt.ylabel('Flux [n/cc/s]')
plt.legend(("NumPy Function","Power Iteration"), loc="upper right")
plt.title("Power Iteration and NumPy Function Flux Solutions")
plt.savefig('hw6_6.pdf')

print("relative difference: "+str(max(abs(np.divide(phi_np-phi_plot,phi_np)))))
print("absolute difference: "+str(max(abs(phi_np-phi_plot))))
